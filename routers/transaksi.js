...
app.post("/", (request, response) => {
    ...
    .then(result => {
        // jika insert transaksi berhasil,
        // lanjut insert data detail transaksinya
...
app.put("/:id_transaksi", async (request, response) => {
    // menampung data yg akan diupdate di data transaksi
    let dataTransaksi = {
        id_member: request.body.id_member,
        tgl: request.body.tgl,
        batas_waktu: request.body.batas_waktu,
    ...
    .then(async (result) => {
        // hapus data detail_transaksi yg lama
        await detail_transaksi.destroy({where: parameter})

        // masukan data detail yg baru
        let detail = request.body.detail_transaksi
        for (let i = 0; i < detail.length; i++) {
...
// endpoint untuk mengubah status bayar
app.get("/bayar/:id_transaksi", (request, response) => {
    ...
    transaksi.update(dataBayar, {where: parameter})
    .then(result => {
        return response.json({
